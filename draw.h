...
void view_and_projection() {
	...
	R = glm::rotate(R, glm::radians(camera.rotate), glm::vec3(0.0f, 1.0f, 0.0f)); // 카메라 공전 위치
	glm::vec4 dir = R * xR * glm::vec4(camera.camera_Pos - camera.camera_Direction, 1.0f); // 동차 좌표를 활용해 원점과 pos와의 벡터에다가 회전을 적용
	view = glm::lookAt(camera.camera_Direction + glm::vec3(dir), camera.camera_Direction, camera.camera_Up); // lookAt을 통한 카메라 설정 (인자값을 통한 카메라 좌표계 생성 후 이에 따른 뷰 변환 행렬을 view에 저장)
